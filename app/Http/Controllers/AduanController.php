...
class AduanController extends Controller
{
   ...
   public function index()
   {
      ...
      $data = Aduan::select('aduan.*', 'status.nama_status', 'status.color')
         ->leftJoin('status', 'aduan.id_status', '=', 'status.id')
         ->where('aduan.deleted', 1)->orderByDesc('aduan.id')->get();
   ...
   public function print($no_aduan)
   {
      ...
      $count = DescAduan::select('desc_aduan.*', 'status.nama_status')
